module Blend2D
  class Context
    ...
    # :nodoc:
    private def get_stroke_options(options : StrokeOptions) : Bool
      LibBlend2D.blContextGetStrokeOptions(self, options).success_or_raise
    end

    # :nodoc:
    private def stroke_options=(options : StrokeOptions)
      LibBlend2D.blContextSetStrokeOptions(self, options).success_or_raise
    end

    # :nodoc:
    private def clip_to_rect_i(rect : LibBlend2D::BLRectI*) : Bool
      LibBlend2D.blContextClipToRectI(self, rect).success_or_raise
    end

    # :nodoc:
    private def clip_to_rect_d(rect : LibBlend2D::BLRect*) : Bool
      LibBlend2D.blContextClipToRectD(self, rect).success_or_raise
    end

    # :nodoc:
    private def restore_clipping : Bool
      LibBlend2D.blContextRestoreClipping(self).success_or_raise
    end

    # :nodoc:
    private def clear_all : Bool
      LibBlend2D.blContextClearAll(self).success_or_raise
    end

    # :nodoc:
    private def clear_rect_i(rect : LibBlend2D::BLRectI*) : Bool
      LibBlend2D.blContextClearRectI(self, rect).success_or_raise
    end

    # :nodoc:
    private def clear_rect_d(rect : LibBlend2D::BLRect*) : Bool
      LibBlend2D.blContextClearRectD(self, rect).success_or_raise
    end

    # :nodoc:
    private def fill_all_rgba64(rgba64 : LibC::ULong) : Bool
      LibBlend2D.blContextFillAllRgba64(self, rgba64).success_or_raise
    end

    # :nodoc:
    private def fill_all_ext(style : Pointer) : Bool
      LibBlend2D.blContextFillAllExt(self, style).success_or_raise
    end

    # :nodoc:
    private def fill_rect_i(rect : LibBlend2D::BLRectI*) : Bool
      LibBlend2D.blContextFillRectI(self, rect).success_or_raise
    end

    # :nodoc:
    private def fill_rect_i_rgba32(rect : LibBlend2D::BLRectI*, rgba32 : UInt32) : Bool
      LibBlend2D.blContextFillRectIRgba32(self, rect, rgba32).success_or_raise
    end

    # :nodoc:
    private def fill_rect_i_rgba64(rect : LibBlend2D::BLRectI*, rgba64 : LibC::ULong) : Bool
      LibBlend2D.blContextFillRectIRgba64(self, rect, rgba64).success_or_raise
    end

    # :nodoc:
    private def fill_rect_i_ext(rect : LibBlend2D::BLRectI*, style : Pointer) : Bool
      LibBlend2D.blContextFillRectIExt(self, rect, style).success_or_raise
    end

    # :nodoc:
    private def fill_rect_d(rect : LibBlend2D::BLRect*) : Bool
      LibBlend2D.blContextFillRectD(self, rect).success_or_raise
    end

    # :nodoc:
    private def fill_rect_d_rgba32(rect : LibBlend2D::BLRect*, rgba32 : UInt32) : Bool
      LibBlend2D.blContextFillRectDRgba32(self, rect, rgba32).success_or_raise
    end

    # :nodoc:
    private def fill_rect_d_rgba64(rect : LibBlend2D::BLRect*, rgba64 : LibC::ULong) : Bool
      LibBlend2D.blContextFillRectDRgba64(self, rect, rgba64).success_or_raise
    end

    # :nodoc:
    private def fill_rect_d_ext(rect : LibBlend2D::BLRect*, style : Pointer) : Bool
      LibBlend2D.blContextFillRectDExt(self, rect, style).success_or_raise
    end

    # :nodoc:
    private def fill_path_d_rgba32(origin : LibBlend2D::BLPoint*, path : Path, rgba32 : UInt32) : Bool
      LibBlend2D.blContextFillPathDRgba32(self, origin, path, rgba32).success_or_raise
    end

    # :nodoc:
    private def fill_path_d_rgba64(origin : LibBlend2D::BLPoint*, path : Path, rgba64 : LibC::ULong) : Bool
      LibBlend2D.blContextFillPathDRgba64(self, origin, path, rgba64).success_or_raise
    end

    # :nodoc:
    private def fill_path_d_ext(origin : LibBlend2D::BLPoint*, path : Path, style : Pointer) : Bool
      LibBlend2D.blContextFillPathDExt(self, origin, path, style).success_or_raise
    end

    # :nodoc:
    private def fill_geometry(type : GeometryType, data : Pointer) : Bool
      LibBlend2D.blContextFillGeometry(self, type, data).success_or_raise
    end

    # :nodoc:
    private def fill_geometry_rgba32(type : GeometryType, data : Pointer, rgba32 : UInt32) : Bool
      LibBlend2D.blContextFillGeometryRgba32(self, type, data, rgba32).success_or_raise
    end

    # :nodoc:
    private def fill_geometry_rgba64(type : GeometryType, data : Pointer, rgba64 : LibC::ULong) : Bool
      LibBlend2D.blContextFillGeometryRgba64(self, type, data, rgba64).success_or_raise
    end

    # :nodoc:
    private def fill_utf8_text_i(origin : LibBlend2D::BLPointI*, font : Font, text : UInt8*, size : Int32) : Bool
      LibBlend2D.blContextFillUtf8TextI(self, origin, font, text, size).success_or_raise
    end

    # :nodoc:
    private def fill_utf8_text_i_rgba32(origin : LibBlend2D::BLPointI*, font : Font, text : UInt8*, size : Int32, rgba32 : UInt32) : Bool
      LibBlend2D.blContextFillUtf8TextIRgba32(self, origin, font, text, size, rgba32).success_or_raise
    end

    # :nodoc:
    private def fill_utf8_text_i_rgba64(origin : LibBlend2D::BLPointI*, font : Font, text : UInt8*, size : Int32, rgba64 : LibC::ULong) : Bool
      LibBlend2D.blContextFillUtf8TextIRgba64(self, origin, font, text, size, rgba64).success_or_raise
    end

    # :nodoc:
    private def fill_utf8_text_i_ext(origin : LibBlend2D::BLPointI*, font : Font, text : UInt8*, size : Int32, style : Pointer) : Bool
      LibBlend2D.blContextFillUtf8TextIExt(self, origin, font, text, size, style).success_or_raise
    end

    # :nodoc:
    private def fill_utf8_text_d_rgba64(origin : LibBlend2D::BLPoint*, font : Font, text : UInt8*, size : Int32, rgba64 : LibC::ULong) : Bool
    ...
    # :nodoc:
    private def fill_glyph_run_i(origin : LibBlend2D::BLPointI*, font : Font, glyphRun : LibBlend2D::BLGlyphRun*) : Bool
      LibBlend2D.blContextFillGlyphRunI(self, origin, font, glyphRun).success_or_raise
    end

    # :nodoc:
    private def fill_glyph_run_i_rgba32(origin : LibBlend2D::BLPointI*, font : Font, glyphRun : LibBlend2D::BLGlyphRun*, rgba32 : UInt32) : Bool
      LibBlend2D.blContextFillGlyphRunIRgba32(self, origin, font, glyphRun, rgba32).success_or_raise
    end

    # :nodoc:
    private def fill_glyph_run_i_rgba64(origin : LibBlend2D::BLPointI*, font : Font, glyphRun : LibBlend2D::BLGlyphRun*, rgba64 : LibC::ULong) : Bool
      LibBlend2D.blContextFillGlyphRunIRgba64(self, origin, font, glyphRun, rgba64).success_or_raise
    end

    # :nodoc:
    private def fill_glyph_run_i_ext(origin : LibBlend2D::BLPointI*, font : Font, glyphRun : LibBlend2D::BLGlyphRun*, style : Pointer) : Bool
      LibBlend2D.blContextFillGlyphRunIExt(self, origin, font, glyphRun, style).success_or_raise
    end

    # :nodoc:
    private def fill_glyph_run_d_rgba64(origin : LibBlend2D::BLPoint*, font : Font, glyphRun : LibBlend2D::BLGlyphRun*, rgba64 : LibC::ULong) : Bool
      LibBlend2D.blContextFillGlyphRunDRgba64(self, origin, font, glyphRun, rgba64).success_or_raise
    end

    # :nodoc:
    private def fill_glyph_run_d_ext(origin : LibBlend2D::BLPoint*, font : Font, glyphRun : LibBlend2D::BLGlyphRun*, style : Pointer) : Bool
      LibBlend2D.blContextFillGlyphRunDExt(self, origin, font, glyphRun, style).success_or_raise
    end

    # :nodoc:
    private def fill_mask_i(origin : LibBlend2D::BLPointI*, mask : Image, maskArea : LibBlend2D::BLRectI*) : Bool
      LibBlend2D.blContextFillMaskI(self, origin, mask, maskArea).success_or_raise
    end

    # :nodoc:
    private def fill_mask_i_rgba32(origin : LibBlend2D::BLPointI*, mask : Image, maskArea : LibBlend2D::BLRectI*, rgba32 : UInt32) : Bool
      LibBlend2D.blContextFillMaskIRgba32(self, origin, mask, maskArea, rgba32).success_or_raise
    end

    # :nodoc:
    private def fill_mask_i_rgba64(origin : LibBlend2D::BLPointI*, mask : Image, maskArea : LibBlend2D::BLRectI*, rgba64 : LibC::ULong) : Bool
      LibBlend2D.blContextFillMaskIRgba64(self, origin, mask, maskArea, rgba64).success_or_raise
    end

    # :nodoc:
    private def fill_mask_i_ext(origin : LibBlend2D::BLPointI*, mask : Image, maskArea : LibBlend2D::BLRectI*, style : Pointer) : Bool
      LibBlend2D.blContextFillMaskIExt(self, origin, mask, maskArea, style).success_or_raise
    end

    # :nodoc:
    private def fill_mask_d(origin : LibBlend2D::BLPoint*, mask : Image, maskArea : LibBlend2D::BLRectI*) : Bool
      LibBlend2D.blContextFillMaskD(self, origin, mask, maskArea).success_or_raise
    end

    # :nodoc:
    private def fill_mask_d_rgba32(origin : LibBlend2D::BLPoint*, mask : Image, maskArea : LibBlend2D::BLRectI*, rgba32 : UInt32) : Bool
      LibBlend2D.blContextFillMaskDRgba32(self, origin, mask, maskArea, rgba32).success_or_raise
    end

    # :nodoc:
    private def fill_mask_d_rgba64(origin : LibBlend2D::BLPoint*, mask : Image, maskArea : LibBlend2D::BLRectI*, rgba64 : LibC::ULong) : Bool
      LibBlend2D.blContextFillMaskDRgba64(self, origin, mask, maskArea, rgba64).success_or_raise
    end

    # :nodoc:
    private def fill_mask_d_ext(origin : LibBlend2D::BLPoint*, mask : Image, maskArea : LibBlend2D::BLRectI*, style : Pointer) : Bool
