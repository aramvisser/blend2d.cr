module Blend2D
  class BitSet
    ...
    # :nodoc:
    def initialize(startBit : UInt32, endBit : UInt32)
    ...
    # :nodoc:
    def get_data(out_ : LibBlend2D::BLBitSetData*) : Bool
      LibBlend2D.blBitSetGetData(self, out_).success_or_raise
    end

    # :nodoc:
    def segment_count : UInt32
      LibBlend2D.blBitSetGetSegmentCount(self)
    end

    # :nodoc:
    def segment_capacity : UInt32
    ...
    # :nodoc:
    def self.subsumes(a : BitSet, b : BitSet) : Int32
      LibBlend2D.blBitSetSubsumes(a, b)
    end

    # :nodoc:
    def self.intersects(a : BitSet, b : BitSet) : Int32
    ...
    # :nodoc:
    def self.equals(a : BitSet, b : BitSet) : Int32
      LibBlend2D.blBitSetEquals(a, b)
    end

    # :nodoc:
    def self.compare(a : BitSet, b : BitSet) : Int32
    ...
    # :nodoc:
    def chop(startBit : UInt32, endBit : UInt32) : Bool
      LibBlend2D.blBitSetChop(self, startBit, endBit).success_or_raise
    end

    # :nodoc:
    def add_bit(bitIndex : UInt32) : Bool
      LibBlend2D.blBitSetAddBit(self, bitIndex).success_or_raise
    end

    # :nodoc:
    def add_range(rangeStartBit : UInt32, rangeEndBit : UInt32) : Bool
      LibBlend2D.blBitSetAddRange(self, rangeStartBit, rangeEndBit).success_or_raise
    end

    # :nodoc:
    def add_words(startWord : UInt32, wordData : UInt32*, wordCount : UInt32) : Bool
    ...
    # :nodoc:
    def clear_range(rangeStartBit : UInt32, rangeEndBit : UInt32) : Bool
