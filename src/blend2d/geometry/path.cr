module Blend2D::Geometry
  class Path < Core(LibBlend2D::BLPathCore)
    ...
    def move_to(p0 : Point)
      move_to point.x, point.y
    end

    def move_to(x0 : Float64, y0 : Float64)
      LibBlend2D.blPathMoveTo(self, x0, y0).success_or_raise
    end

    def line_to(p1 : Point)
      lint_to line.x, line.y
    end

    def line_to(x1 : Float64, y1 : Float64)
      LibBlend2D.blPathLineTo(self, x1, y1).success_or_raise
    end

    def poly_to(poly : Array(Point))
      LibBlend2D.blPathPolyTo(self, poly.to_unsafe.as(Pointer(LibBlend2D::BLPoint)), poly.size).success_or_raise
    end

    def quad_to(p1 : Point, p2 : Point)
      quad_to p1.x, p1.y, p2.x, p2.y
    end

    def quad_to(x1 : Float64, y1 : Float64, x2 : Float64, y2 : Float64)
      LibBlend2D.blPathQuadTo(self, x1, y1, x2, y2).success_or_raise
    end

    def conic_to(p1 : Point, p2 : Point, w : Float64)
      conic_to p1.x, p1.y, p2.x, p2.y, w
    end

    def conic_to(x1 : Float64, y1 : Float64, x2 : Float64, y2 : Float64, w : Float64)
      LibBlend2D.blPathConicTo(self, x1, y1, x2, y2, w).success_or_raise
    end

    def cubic_to(p1 : Point, p2 : Point, p3 : Point)
      conic_to p1.x, p1.y, p2.x, p2.y, p3.x, p3.y
    end

    def cubic_to(x1 : Float64, y1 : Float64, x2 : Float64, y2 : Float64, x3 : Float64, y3 : Float64)
      LibBlend2D.blPathCubicTo(self, x1, y1, x2, y2, x3, y3).success_or_raise
    end

    def smooth_quad_to(p2 : Point)
      smooth_quad_to p2.x, p2.y
    end

    def smooth_quad_to(x2 : Float64, y2 : Float64)
      LibBlend2D.blPathSmoothQuadTo(self, x2, y2).success_or_raise
    end

    def smooth_cubic_to(p2 : Point, p3 : Point)
      smooth_cubic_to p2.x, p2.y, p3.x, p3.y
    end

    def smooth_cubic_to(x2 : Float64, y2 : Float64, x3 : Float64, y3 : Float64)
      LibBlend2D.blPathSmoothCubicTo(self, x2, y2, x3, y3).success_or_raise
    end

    def arc_to(c : Point, r : Point, start : Float64, sweep : Float64, force_move_to) : Bool
      arc_to c.x, c.y, r.x, r.y, start, sweep, force_move_to
    end

    def arc_to(x : Float64, y : Float64, rx : Float64, ry : Float64, start : Float64, sweep : Float64, force_move_to) : Bool
      LibBlend2D.blPathArcTo(self, x, y, rx, ry, start, sweep, forceMoveTo).success_or_raise
    end

    def arc_quadrant_to(p1 : Point, p2 : Point)
      arc_quadrant_to p1.x, p1.y, p2.x, p2.y
    end

    def arc_quadrant_to(x1 : Float64, y1 : Float64, x2 : Float64, y2 : Float64)
      LibBlend2D.blPathArcQuadrantTo(self, x1, y1, x2, y2).success_or_raise
    end

    def elliptic_arc_to(rp : Point, x_axis_rotation : Float64, large_arc_flag, sweep_flag : Bool, p1 : Point) : Bool
      LibBlend2D.blPathEllipticArcTo(self, rp.x, rp.y, x_axis_rotation, large_arc_flag, sweep_flag, p1.x, p1.y).success_or_raise
    end

    def elliptic_arc_to(rx : Float64, ry : Float64, x_axis_rotation : Float64, large_arc_flag, sweep_flag : Bool, x1 : Float64, y1 : Float64) : Bool
    ...
    def add_box(x1 : Float64, y1 : Float64, x2 : Float64, y2 : Float64, dir : GeometryDirection = GeometryDirection::CW)
      add_box Box.new(x1, y1, x2, y2), dir
    end

    def add_box(box : Box | BoxI, dir : GeometryDirection = GeometryDirection::CW)
      case box
      in Box
    ...
    def add_rect(x : Float64, y : Float64, w : Float64, h : Float64, dir : GeometryDirection = GeometryDirection::CW)
      add_rect Rect.new(x, y, w, h), dir
    end

    def add_rect(rect : Rect | RectI, dir : GeometryDirection = GeometryDirection::CW)
      case rect
      in Rect
    ...
    def add_geometry(geometry : Geometry::Core, m : Matrix2D, dir : GeometryDirection = Geometry::CW)
      LibBlend2D.blPathAddGeometry(self, geometry.type, geometry.to_unsafe, m, dir).success_or_raise
    end

    def add_geometry(geometry : Geometry::Core, dir : GeometryDirection = GeometryDirection::CW)
      LibBlend2D.blPathAddGeometry(self, geometry.type, geometry.to_unsafe, nil, dir).success_or_raise
    end

    def add_circle(circle : Circle, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry circle, dir
    end

    def add_circle(circle : Circle, transform : Matrix2D, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry circle, transform, dir
    end

    def add_ellipse(ellipse : Ellipse, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry ellipse, dir
    end

    def add_ellipse(ellipse : Ellipse, transform : Matrix2D, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry ellipse, transform, dir
    end

    def add_round_rect(round_rect : RoundRect, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry round_rect, dir
    end

    def add_round_rect(round_rect : RoundRect, transform : Matrix2D, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry round_rect, transform, dir
    end

    def add_arc(arc : Arc, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry arc, dir
    end

    def add_arc(arc : Arc, transform : Matrix2D, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry arc, transform, dir
    end

    def add_chord(chord : Chord, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry chord, dir
    end

    def add_chord(chord : Chord, transform : Matrix2D, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry chord, transform, dir
    end

    def add_pie(pie : Pie, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry pie, dir
    end

    def add_pie(pie : Pie, transform : Matrix2D, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry pie, transform, dir
    end

    def add_line(line : Line, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry line, dir
    end

    def add_line(line : Line, transform : Matrix2D, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry line, transform, dir
    end

    def add_triangle(triangle : Triangle, dir : GeometryDirection = GeometryDirection::CW)
      add_geometry triangle, dir
    end

    def add_triangle(triangle : Triangle, transform : Matrix2D, dir : GeometryDirection = GeometryDirection::CW)
    ...
    def add_stroked_path(other : Path)
      LibBlend2D.blPathAddStrokedPath(self, other, nil, nil, nil).success_or_raise
    end

    def add_stroked_path(other : Path, range : Range(Int32, Int32))
      LibBlend2D.blPathAddStrokedPath(self, other, BLRange.new(range), nil, nil).success_or_raise
    end

    def add_stroked_path(other : Path, range : Range(Int32, Int32), options : StrokeOptions? = nil, approx : ApproximationOptions? = nil)
